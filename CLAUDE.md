...
- **Keyboard shortcuts** live in one registry (`client/src/lib/keybindings.ts`): action, label, group, default chord. Chords are canonical strings (`Mod+Shift+F`, `Mod+Enter`) where `Mod` is ⌘/Ctrl. The bindings persist in the store (`keybindings`, localStorage `monolith-keybindings`) and are rebindable in Settings; `useShortcuts` (`client/src/hooks/useShortcuts.ts`) is the single dispatcher, listening in the **capture** phase so a bound chord outranks CodeMirror's keymap. Every visible hint (rail tooltips, Compile/Save buttons, palette, empty states) reads its label from the registry with `formatChord`, so a rebind relabels the whole UI. Escape is deliberately outside the registry.
