...
- **client/src/components/panels/** — rail panels: Files, Outline, Scope (In-scope), References, Plots, History (`HistoryPanel.tsx` + `VersionDetail.tsx`), Projects, plus the symbol/snippet drawer content
...
- **server/src/routes/** — API handlers (compile, files, projects, synctex, scope, versions)
- **server/src/services/** — Business logic (tectonic spawning, latexml HTML rendering, chokidar file watcher, synctex parsing, scope resolution, per-project version store)
